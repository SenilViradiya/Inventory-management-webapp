  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (alertsResponse.success) {
        setAlerts(alertsResponse.data.slice(0, 5)); // Get latest 5 alerts
      }
      ...
      if (productsResponse.success) {
        const lowStock = productsResponse.data.filter(
          (product: Product) => product.quantity <= product.reorderLevel
        );
        setLowStockProducts(lowStock.slice(0, 5)); // Get first 5 low stock products
      }
    } catch (error) {
