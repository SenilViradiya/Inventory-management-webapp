...
export const categoriesAPI = {
  ...
  getStockSummary: (params?: {
    ...
    if (format && format !== 'json') {
      // For exports, return blob
      return api.get('/categories/stock-summary', {
        params: { ...otherParams, format },
        responseType: 'blob'
      });
    }
    
    return api.get<ApiResponse<any>>('/categories/stock-summary', { params: otherParams });
  }
};
...
export const developerAPI = {
  ...
  ingestMetric: (data: {
    ...
    value?: number;
    details?: any;
  }) => api.post<ApiResponse<{ success: boolean }>>('/dev-analytics/ingest', data),

  // Get app summary metrics
  getAppSummary: (app: string, startDate?: string, endDate?: string) =>
    api.get<ApiResponse<DeveloperAppSummary>>(`/developer/app-summary/${app}`, { params: { startDate, endDate } }),

  // Get system metrics (mock for now, can be extended)
  getSystemMetrics: () =>
    api.get<ApiResponse<SystemMetrics>>('/developer/system-metrics'),

  // Get API endpoints metrics
  getAPIEndpoints: () =>
    api.get<ApiResponse<APIEndpoint[]>>('/developer/api-endpoints'),
  // Store analytics methods
  getAllStores: () =>
    api.get<ApiResponse<{ _id: string; name: string }[]>>('/shops'),
  getStoreAnalytics: (storeId: string) =>
    api.get<ApiResponse<any>>(`/developer/store-analytics/${storeId}`),
  getLatestRequests: (storeId: string) =>
    api.get<ApiResponse<any[]>>(`/developer/store-requests/${storeId}`),
};
export default api;
